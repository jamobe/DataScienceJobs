...
if __name__ == "__main__":
    ...
    country_region_dict = {'UK': list(loc_UK), 'Germany': list(loc_GER), 'USA': list(loc_USA), None: None,
                           'Netherlands': 'North Holland', 'Australia': 'Tasmania', 'Sweden': None,
                           'Belgium': 'Brussels', 'Ireland': 'Leinster', 'Switzerland': 'Zurich',
                           'Lithuania': 'Vilnius', 'France': 'Ile-de-France', 'Spain': 'Barcelona',
                           'Poland': 'Masovia', 'Canada': 'Canada'}
    ...
    length = pd.read_sql(''' 
    SELECT count(*) FROM all_data ''', engine).iloc[0, 0]
    if count == length:
    ...
    SELECT count(*) FROM all_data 
    WHERE salary_type = 'yearly' 
    AND salary_high_euros/salary_low_euros > 3 ''', engine).iloc[0, 0]
