...
if __name__ == "__main__":
    ...
    if exists is True:
        ...
        if myinput == 'yes':
            engine.execute('''
            INSERT INTO all_data (job_title, ref_code, company, description, salary, salary_low, salary_high, currency, 
            salary_average, salary_low_euros, salary_high_euros, salary_average_euros, salary_type, location, jobtype, 
            posted_date, extraction_date, country, region, url, language)
            SELECT job_title, ref_code, company, description, salary, salary_low, salary_high, currency, salary_average, 
            salary_low_euros, salary_high_euros, salary_average_euros, salary_type, location, jobtype, posted_date, 
            extraction_date, country, region, url, language FROM landing
            ''')
    ...
    test_sal = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'test' 
    AND salary_average > 0''', engine)

    train_sal = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'train' 
    AND salary_average > 0''', engine)

    test_UK = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'test' 
    AND salary_average > 0 
    AND country = 'UK' ''', engine)

    train_UK = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'train' 
    AND salary_average > 0 
    AND country = 'UK' ''', engine)

    test_GER = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'test' 
    AND salary_average > 0 
    AND country = 'Germany' ''', engine)

    train_GER = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'train' 
    AND salary_average > 0 
    AND country = 'Germany' ''', engine)

    test_USA = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'test' 
    AND salary_average > 0 
    AND country = 'USA' ''', engine)

    train_USA = pd.read_sql('''
    SELECT count(train_test_label) from all_data 
    WHERE train_test_label = 'train' 
    AND salary_average > 0 
    AND country = 'USA' ''', engine)
    ...
    print("Proportion of test samples with salary: {:.1%} ".format(test_sal.iloc[0, 0]/(train_sal.iloc[0, 0] +
                                                                                        test_sal.iloc[0, 0])))
    print("")
    ...
    print("Proportion of test samples with salary UK: {:.1%} ".format(test_UK.iloc[0, 0]/(train_UK.iloc[0, 0] +
                                                                                          test_UK.iloc[0, 0])))
    print("")
    ...
    print("Proportion of test samples with salary  Germany: {:.1%} ".format(test_GER.iloc[0, 0]/(train_GER.iloc[0, 0] +
                                                                                                 test_GER.iloc[0, 0])))
    print("")
    ...
    print("Proportion of test samples with salary  USA: {:.1%} ".format(test_USA.iloc[0, 0]/(train_USA.iloc[0, 0] +
                                                                                             test_USA.iloc[0, 0])))
